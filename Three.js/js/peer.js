...
;(function (exports) {
  ...
  Unpacker.prototype.unpack_uint64 = function () {
    ...
    var uint64 =
      ((((((bytes[0] * 256 + bytes[1]) * 256 + bytes[2]) * 256 + bytes[3]) * 256 + bytes[4]) * 256 + bytes[5]) *
        256 +
        bytes[6]) *
        256 +
      bytes[7]
  ...
  exports.RTCPeerConnection =
    window.mozRTCPeerConnection || window.webkitRTCPeerConnection || window.RTCPeerConnection
  ...
  Peer.prototype.listAllPeers = function (cb) {
    ...
    var protocol = this.options.secure ? 'https://' : 'http://'
    var url =
      protocol + this.options.host + ':' + this.options.port + this.options.path + this.options.key + '/peers'
    var queryString = '?ts=' + new Date().getTime() + '' + Math.random()
    url += queryString
  ...
  Negotiator._getPeerConnection = function (connection, options) {
    if (!Negotiator.pcs[connection.type]) {
      util.error(
        connection.type + ' is not a valid connection type. Maybe you overrode the `type` property somewhere.'
      )
    }

...
function ShaderParticleEmitter(options) {
  ...
  that.type =
    options.type === 'cube' || options.type === 'sphere' || options.type === 'disk' ? options.type : 'cube'

  that.position = options.position instanceof THREE.Vector3 ? options.position : new THREE.Vector3()
  that.positionSpread =
    options.positionSpread instanceof THREE.Vector3 ? options.positionSpread : new THREE.Vector3()
  ...
  that.radiusScale =
    options.radiusScale instanceof THREE.Vector3 ? options.radiusScale : new THREE.Vector3(1, 1, 1)

  that.acceleration = options.acceleration instanceof THREE.Vector3 ? options.acceleration : new THREE.Vector3()
  that.accelerationSpread =
    options.accelerationSpread instanceof THREE.Vector3 ? options.accelerationSpread : new THREE.Vector3()

  that.velocity = options.velocity instanceof THREE.Vector3 ? options.velocity : new THREE.Vector3()
  that.velocitySpread =
    options.velocitySpread instanceof THREE.Vector3 ? options.velocitySpread : new THREE.Vector3()
  ...
  that.colorStartSpread =
    options.colorStartSpread instanceof THREE.Vector3 ? options.colorStartSpread : new THREE.Vector3(0, 0, 0)
...
ShaderParticleEmitter.prototype = {
  ...
  _resetParticle: function (i) {
    ...
    // If there is a position spread, then get a new position based on this spread.
      ...
      that._randomizeExistingVector3OnSphere(
        particlePosition,
        that.position,
        that.radius,
        that.radiusSpread,
      ...
      that._randomizeExistingVelocityVector3OnSphere(
        particleVelocity,
        that.position,
        particlePosition,
        that.speed,
      ...
      that._randomizeExistingVector3OnDisk(
        particlePosition,
        that.position,
        that.radius,
        that.radiusSpread,
  ...
  _randomizeExistingVector3OnSphere: function (v, base, radius, radiusSpread, radiusScale) {
    ...
    v.set(r * Math.cos(t) * randomRadius, r * Math.sin(t) * randomRadius, z * randomRadius).multiply(radiusScale)
  ...
  _randomizeExistingVector3OnDisk: function (v, base, radius, radiusSpread, radiusScale) {
    ...
    v.set(Math.cos(t), Math.sin(t), 0).multiplyScalar(randomRadius)
  ...
  _randomizeExistingVelocityVector3OnSphere: function (v, base, position, speed, speedSpread) {
    v.copy(position).sub(base).normalize().multiplyScalar(this._randomFloat(speed, speedSpread))
  },

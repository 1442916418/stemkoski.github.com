...
ShaderParticleGroup.prototype = {
  ...
  _randomVector3OnDisk: function (base, radius, radiusSpread, radiusScale) {
    var t = 6.2832 * Math.random()
    var vec = new THREE.Vector3(Math.cos(t), Math.sin(t), 0).multiplyScalar(
      this._randomFloat(radius, radiusSpread)
    )
  ...
  addEmitter: function (emitter) {
    ...
    if (emitter.duration) {
      emitter.numParticles =
        (emitter.particlesPerSecond *
          (that.maxAge < emitter.emitterDuration ? that.maxAge : emitter.emitterDuration)) |
        0
    } else {
    ...
    for (var i = start; i < end; ++i) {
      if (emitter.type === 'sphere') {
        vertices[i] = that._randomVector3OnSphere(
          emitter.position,
          emitter.radius,
          emitter.radiusSpread,
          emitter.radiusScale
        )
        velocity[i] = that._randomVelocityVector3OnSphere(
          vertices[i],
          emitter.position,
          emitter.speed,
          emitter.speedSpread
        ...
        vertices[i] = that._randomVector3OnDisk(
          emitter.position,
          emitter.radius,
          emitter.radiusSpread,
          emitter.radiusScale

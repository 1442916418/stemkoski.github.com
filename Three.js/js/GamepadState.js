...
GamepadState = function () {
  ...
  this.update = function update() {
    ...
    for (var i = 0; i < 16; i++) {
      ...
      if (button.pressed && !button.active) button.down = true

      // is this the first update where this button was released?
      if (!button.pressed && button.active) button.up = true
  ...
  this.down = function down(buttonName) {
    if (this.status.hasOwnProperty(buttonName)) return this.status[buttonName].down
    else console.warn('Gamepad: no button mapped to', buttonName)
  }

  this.pressed = function pressed(buttonName) {
    if (this.status.hasOwnProperty(buttonName)) return this.status[buttonName].pressed
    else console.warn('Gamepad: no button mapped to', buttonName)
  }

  this.up = function up(buttonName) {
    if (this.status.hasOwnProperty(buttonName)) return this.status[buttonName].up
    else console.warn('Gamepad: no button mapped to', buttonName)
  }

  this.value = function value(axisName) {
    if (axisName == 'leftTrigger' || axisName == 'rightTrigger') return this.status[axisName].pressed
    else if (this.status.hasOwnProperty(axisName)) return this.status[axisName].value
    else console.warn('Gamepad: no axis mapped to', axisName)
  }

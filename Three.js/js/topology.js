...
setParameters = function (obj, params) {
  ...
  for (var arg in obj) if (params[arg] !== undefined) obj[arg] = params[arg]
}
...
Array.prototype.changeData = function (oldData, newData) {
  for (var i = 0; i < this.length; i++) if (this[i] == oldData) this[i] = newData
}
...
TOPOLOGY.Topology.prototype.add = function (obj) {
  if (obj.hasOwnProperty('type')) this[obj.type][obj.ID] = obj
  else console.log('Topology.add: unknown object type')
}
...
TOPOLOGY.Topology.prototype.edgeIDWithVertices = function (va, vb) {
  ...
  for (var i = 0; i < edgeCandidateIDs.length; i++) {
    ...
    if (this.edge[edgeIndex].vertexIDs.indexOf(vb) != -1) return edgeIndex
  }
...
TOPOLOGY.Topology.prototype.edgeIDWithFaces = function (fa, fb) {
  ...
  for (var i = 0; i < edgeCandidateIDs.length; i++) {
    var edgeIndex = edgeCandidateIDs[i]
    if (this.edge[edgeIndex].faceIDs.indexOf(fb) != -1) return edgeIndex
  }
...
TOPOLOGY.Topology.prototype.computeCenters = function () {
  ...
  for (var i = 0; i < this.face.length; i++) {
    ...
    for (var v = 0; v < face.vertexIDs.length; v++) face.center.add(this.vertex[face.vertexIDs[v]].vector3)
...
TOPOLOGY.Topology.prototype.textQuery = function (type, ID) {
  ...
  var info = type + ' ' + 'ID ' + ID + '. '
  if (!(type == 'vertex')) info += 'Adj vertices: [' + array.vertexIDs.toString() + ']. '
  if (!(type == 'edge')) info += 'Adj edges: [' + array.edgeIDs.toString() + ']. '
  if (!(type == 'face')) info += 'Adj faces: [' + array.faceIDs.toString() + ']. '
  return info
}
...
TOPOLOGY.Topology.prototype.retriangulate = function (type, ID, vec3) {
  if (type == 'face') this.retriangulateFace(ID, vec3)
  else if (type == 'edge') this.retriangulateEdge(ID, vec3)
  else console.log('Topology.retriangulate: unknown type')
}
...
TOPOLOGY.createFromGeometry = function (geometry) {
  ...
  for (var faceIndex = 0; faceIndex < geometry.faces.length; faceIndex++) {
    ...
    if (face instanceof THREE.Face4) vertexData.push(face['d'])
